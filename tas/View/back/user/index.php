...
<body>
                    ...
                    <ul class="topbar-menu d-flex align-items-center">
                        ...
                        <li class="dropdown">
                            <a class="nav-link dropdown-toggle nav-user me-0 waves-effect waves-light"
                                data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="false" aria-expanded="false">
                                <img src="../../../uploads/<?= $_SESSION['profilePicture'] ?>" alt="user-image" class="rounded-circle">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($users as $user) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <!-- Delete Icon -->
                                                            <a href="index.php?id=<?= $user['id'] ?>&action=delete" class="action-icon">
                                                                <i data-feather="trash"></i>
                                                            </a>

                                                            <!-- Edit Icon -->
                                                            <a class="action-icon" data-bs-toggle="modal" data-bs-target="#update-modal"
                                                                data-id="<?= $user['id'] ?>"
                                                                data-name="<?= $user['name'] ?>"
                                                            ...
                                                            <?php if ($user['isBanned']) : ?>
                                                                <a href="../auth/unban-user.php?id=<?= $user['id'] ?>" class="action-icon text-success" title="Unban User">
                                                                    <i data-feather="unlock"></i>
                                                                </a>
                                                            <?php else : ?>
                                                                <a href="../auth/ban-user.php?id=<?= $user['id'] ?>" class="action-icon text-danger" title="Ban User">
    ...
    <script>
        function filterTable() {
            ...
            // Loop through table rows, starting from 1 (skip headers)
            for (let i = 1; i < rows.length; i++) {
                const name = rows[i].getElementsByTagName('td')[0]?.innerText.toLowerCase() || '';
                const email = rows[i].getElementsByTagName('td')[1]?.innerText.toLowerCase() || '';
                const phone = rows[i].getElementsByTagName('td')[2]?.innerText.toLowerCase() || '';
                const role = rows[i].getElementsByTagName('td')[3]?.innerText.toLowerCase() || '';

                // Check if the query matches any column value
                if (
                    name.includes(input) ||
