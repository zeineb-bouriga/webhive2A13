...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
    ...
    if ($payment_method == "paypal" && empty($payment_details)) {
        $errors[] = "Veuillez entrer les détails de votre compte PayPal.";
    }
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO orders (user_id, name, address, phone, email, payment_method, status) VALUES (?, ?, ?, ?, ?, ?, 'pending')");
        $stmt->execute([$user_id, $name, $address, $phone, $email, $payment_method]);
    ...
    <script>
        function togglePaymentDetails() {
            ...
            if (paymentMethod === "paypal") {
                ...
                paymentDetailsDiv.style.display = "block";
                document.getElementById("payment_details").placeholder = "Entrez votre numéro de carte de crédit";
            } else {
